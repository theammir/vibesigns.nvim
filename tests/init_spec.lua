...
local gv = require('gitsigns-vibecoded')

local function refresh_sync(bufnr)
  gv.refresh(bufnr)
  vim.wait(5000, function()
    return #vim.api.nvim_buf_get_extmarks(bufnr, require('gitsigns-vibecoded.signs').ns, 0, -1, {}) > 0
...
describe('gitsigns-vibecoded end-to-end', function()
  it('signs committed agent lines, ignores human and modified lines', function()
    ...
    repo.commit(dir, git, 'f.txt', { 'human1', 'agentline' }, { coauthor = 'noreply@anthropic.com' })
    ...
    local ns = require('gitsigns-vibecoded.signs').ns
  ...
  it('signs agent lines for a file in a subdirectory', function()
    ...
    repo.commit(dir, git, 'sub/f.txt', { 'human1', 'agentline' }, { coauthor = 'noreply@anthropic.com' })
  ...
  it('setup with enabled=false places no signs', function()
    ...
    require('gitsigns-vibecoded.blame')._reset_cache()
